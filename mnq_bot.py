...
def kis_min_1600(symbol=None, days=12):
    ...
    for page in range(12):                       # 최대 12회 페이징
        ...
        if r.get("rt_cd") != "0":
            ...
            # 첫 페이지에서 실패하면 다음 파라미터 조합으로 재시도
            if page == 0 and _vi + 1 < len(VARIANTS):
                log(f"분봉 조합 {_vi+1} 실패({_lasterr[:60]}) — 다음 조합 시도")
                _vi += 1
                continue
            raise RuntimeError(_lasterr)
        rows = r.get("output2") or []
