...
def kis_min_1600(symbol=None, days=12):
    ...
    for _vi, _vp in enumerate(VARIANTS):
        ...
        if len(by_day) >= 3:
            ...
            break
        log(f"분봉 조합 {_vi+1} 실패({(_lasterr or '행 부족')[:60]}) — 다음 조합 시도")
        time.sleep(0.5)
...
def kis_now_price(symbol=None):
    """한투 해외선물 실시간 현재가  tr_id=HHDFC55010100
       분봉이 10분쯤 지연되므로 마감 직후 판정에 쓴다."""
    sym = symbol or active_contract()
    r = _kis_get("/uapi/overseas-futureoption/v1/quotations/inquire-price",
                 "HHDFC55010100", {"SRS_CD": sym})
    if r.get("rt_cd") != "0":
        raise RuntimeError(r.get("msg1", "현재가 조회 실패"))
    o = r.get("output2") or r.get("output1") or r.get("output") or {}
    if isinstance(o, list):
        o = o[0] if o else {}
    for k in ("last", "prpr", "ovrs_nmix_prpr", "stck_prpr", "p_last"):
        v = o.get(k)
        if v not in (None, "", "0"):
            try:
                px = float(v)
            ...
            if px > 5000:
                return px
    raise RuntimeError(f"현재가 파싱 실패: {str(o)[:150]}")
