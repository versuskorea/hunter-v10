...
def kis_min_1600(symbol=None, days=12):
    ...
    # 한투가 날짜 형식을 까다롭게 본다 — 여러 조합을 순서대로 시도
    VARIANTS = [
        {"START_DATE_TIME": "", "CLOSE_DATE_TIME": ""},                       # 최신부터
        {"START_DATE_TIME": "", "CLOSE_DATE_TIME": _kst.strftime("%Y%m%d")},  # 한국 날짜
        {"START_DATE_TIME": "", "CLOSE_DATE_TIME": _et.strftime("%Y%m%d")},   # ET 날짜
        {"START_DATE_TIME": (_kst - timedelta(days=25)).strftime("%Y%m%d"),
         "CLOSE_DATE_TIME": _kst.strftime("%Y%m%d")},                          # 범위 지정
    ]
    _lasterr = ""
    by_day, page = {}, 0
    for _vi, _vp in enumerate(VARIANTS):
        ...
        for page in range(12):                       # 최대 12회 페이징
            ...
            if r.get("rt_cd") != "0":
                ...
                break
            # ── 봉 배열 찾기 ──
            # 한투는 output1/output2 배치가 일정하지 않고, 봉이 없을 땐
            # 메타(index_key·last_n_cnt)만 한 줄 돌려준다. data_date 가 있는
            # 행만 봉으로 취급하고, 없는 행은 페이징 메타로 쓴다.
            def _is_bar(x):
            ...
            for _key in ("output2", "output1", "output"):
                ...
                for _x in _lst:
                    if _is_bar(_x):
                        _bars.append(_x)
                    elif isinstance(_x, dict) and not _meta:
                        _meta = _x
            rows = _bars
            globals()["_KIS_META"] = _meta
            if not rows:
                _cnt = _meta.get("last_n_cnt") if isinstance(_meta, dict) else None
                _lasterr = f"봉 없음 (last_n_cnt={_cnt})" if _cnt is not None else (_lasterr or "응답 행 없음")
                break
            ok_pages += 1
            # 진단: 첫 페이지 응답 모양을 남긴다 (필드명·시각 범위·페이징 키)
            if page == 0:
                _r0 = rows[0]
                _ts = [str(x.get("data_time") or "")[:4] for x in rows[:3]]
                _m0 = globals().get("_KIS_META") or {}
                _rl = rows[-1]
                _now_et = et_now().strftime("%m-%d %H:%M")
                _now_k  = (datetime.utcnow()+timedelta(hours=9)).strftime("%m-%d %H:%M")
                log(f"분봉 응답 {len(rows)}행 · 키 {sorted(_r0.keys())[:8]} · index_key={'Y' if _m0.get('index_key') else 'N'}")
                log(f"  첫 행 {_r0.get('data_date')} {_r0.get('data_time')} 종가 {_r0.get('last_price') or _r0.get('data_close')} · "
                    f"끝 행 {_rl.get('data_date')} {_rl.get('data_time')} 종가 {_rl.get('last_price') or _rl.get('data_close')}")
                log(f"  지금 ET {_now_et} · KST {_now_k}  ← 봉 시각이 어느 쪽과 맞는지 확인")
            for x in rows:
            ...
            if len(by_day) >= days + 2:
                break
            _m = globals().get("_KIS_META") or {}
            idx_key = str(_m.get("index_key") or "")
            if not idx_key:
                break
            qtp = "P"
            time.sleep(0.25)                          # 한투 호출 간격 (속도 제한 회피)
        if len(by_day) >= 3:
            if _vi: log(f"분봉 조합 {_vi+1}로 성공")
            break
        log(f"분봉 조합 {_vi+1} 실패({(_lasterr or '행 부족')[:60]}) · 수집 {len(by_day)}일 · 페이지 {page+1} — 다음 조합 시도")
        time.sleep(0.5)
    ...
    # ── 분봉 지연 보정 ──
    # 한투 분봉은 10분쯤 늦게 올라온다. 이미 16:00 ET가 지났는데 마지막 봉이
    # 15:59 이전이면 실시간 현재가를 당일 종가로 쓴다 (마감 직후 = 사실상 종가).
    if LIVE_CLOSE and not FORCE_RUN:
        _e = et_now()
        _today = _e.strftime("%Y-%m-%d")
        _stale = by_day[newest][0] < 15*60 + 59
        if _e.hour >= 16 and (newest != _today or _stale):
            try:
...
def quote_test():
    ...
    try:
        ...
        out.append(f"✅ 한투 분봉 {len(b)}일 · 최신 {b[-1][0]} {b[-1][1]:,.2f} (마지막 봉 {hh:02d}:{mm:02d} ET)")
    except Exception as e:
