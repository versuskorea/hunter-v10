    ...
    for _vi, _vp in enumerate(VARIANTS):
        ...
        for page in range(12):                       # 최대 12회 페이징
            ...
            if page == 0:
                ...
                _o1 = r.get("output1") or {}
                if isinstance(_o1, list): _o1 = _o1[0] if _o1 else {}
                log(f"분봉 응답 {len(rows)}행 · 키 {sorted(_r0.keys())[:8]} · "
                    f"첫 시각 {_ts} · index_key={'Y' if (isinstance(_o1,dict) and _o1.get('index_key')) else 'N'}")
            for x in rows:
